...
def calcCoeffs(sideId, data, pixToRectFunc, polynomialDegree=3):
  ...
  non_zero_data = data[..., 1:].reshape(-1, 2)[non_zero_indices].astype(np.float32) / 255
  ...
  non_zero_pixel_coordinates = coordinates.reshape(-1, 1, 2)[non_zero_indices].astype(np.float32)
